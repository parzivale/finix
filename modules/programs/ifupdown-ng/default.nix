...
in
{
  ...
  config = lib.mkIf cfg.enable {
    ...
    # `post` was finit's own notion of a stop action on a task. The contract has a side of the
    # graph for exactly this: a unit attached to the latch runs on the way down, and nowhere
    # else, on every implementation.
